...
if (isGetCookie) {
  !(async () => {
    ...
    if ($.setdata(JSON.stringify(session), $.signKeySX)) {
      $.subt = `获取会话: 成功!`
    } else {
      $.subt = `获取会话: 失败!`
    }
...
async function key() {
    return new Promise((resolve) => {
        signbody = JSON.parse($.getdata($.signKeySX)).body;
        signheaders = JSON.parse($.getdata($.signKeySX)).headers;

        url_t = JSON.parse($.getdata($.signKeySX)).url;
        const url = {
            url: 'http://www.txtnovel.vip/plugin.php?id=dsu_paulsign:sign&operation=qiandao&infloat=0&inajax=0&mobile=yes',
            headers: {
        ...
        $.post(url, (err, resp, data) => {
            ...
            if (res.Code == 1) {
                sub = `签到结果: 签到成功`
                det += `积分增加:${res.AddIntegral}` + `\n`
                det += `已连续签到:${res.NeedDays}` + `/7天`
            } else {
