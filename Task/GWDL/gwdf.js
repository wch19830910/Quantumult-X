...
const ERR = MYERR();
$.cookie = $.read("evil_billCookie");
$.body = $.read("evil_billBody");
...
!(async () => {
    ...
    .catch((err) => {
        if (err instanceof ERR.ParseError) {
            $.notify("国网电费", "❌ 解析数据出现错误", err.message);
        } else if (err instanceof ERR.EventError) {
            $.notify("国网电费", "❌ 请尝试重新获取Cookie", err.message);
        } else {
            $.notify(
                "国网电费",
                "❌ 出现错误",
...
function getCookie() {
    if (
        ...
        $request.url.match(/elec\/account\/query/)
    ) {
        const cookie = $request.headers["Cookie"];
        $.log(cookie);
        $.write(cookie, "evil_billCookie");
        const body = $request.body;
        $.log(body);
        $.write(body, "evil_billBody");
        $.notify("国网电费", "", "获取Cookie成功🎉");
    }
}
